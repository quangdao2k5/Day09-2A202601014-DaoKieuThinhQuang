...
## 10. Implementation trong repository này

Pipeline sử dụng custom Python A2A framework không có dependency runtime bên ngoài. Domain agents tính toán deterministic; `qwen/qwen3-8b` (8,2B parameters) qua OpenRouter chỉ audit consistency sau khi output đã pass verifier.
...
Điền `OPENROUTER_API_KEY` vào `.env`. Không commit hoặc đưa file này vào artifact.
...
Lượt kiểm tra deterministic, không gọi provider:

```bash
python3 -m ecommerce_agents.cli run
```

Lượt chạy cuối có model audit và trace thật:

```bash
python3 -m ecommerce_agents.cli run --llm-audit
```

Mỗi run ghi đè trace cũ. Kiểm tra output:
...
### Tạo file nộp chấm điểm

Commit source/input trước, sau đó chạy:
